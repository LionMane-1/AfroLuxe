    ...
    try {
      ...
      const config = {
        ...
        config: {
          ...
          systemInstruction: `You are Melanie, a Senior Growth Strategist at Afro Hair Lux Marketing (AHLM).
    ...
    <>
      <AnimatePresence>
        {isOpen ? (
          ...
          >
            <div className="bg-primary p-4 flex justify-between items-center shrink-0">
              <div className="flex items-center gap-3 text-white">
                ...
                <div>
                    ...
                    <p className="text-[10px] text-blue-200 uppercase tracking-wider font-medium">Growth Strategist (B2B)</p>
                </div>
              </div>
              <button onClick={() => setIsOpen(false)} className="text-white/80 hover:text-white p-1.5 rounded-full"><Minimize2 className="w-4 h-4" /></button>
            </div>
            ...
            <div ref={scrollRef} className="flex-grow overflow-y-auto p-4 space-y-4 bg-slate-50 dark:bg-slate-950/50">
                {messages.length === 0 && !isConnected && (
                    <div className="text-center mt-12 opacity-50">
                        ...
                        <p className="text-sm">Talk to Melanie about your salon's growth strategy.</p>
                    </div>
                )}
                {messages.map((msg, idx) => (
                    <motion.div key={idx} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                        ...
                        <div className={`p-3 rounded-2xl max-w-[80%] text-sm leading-relaxed ${msg.role === 'user' ? 'bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 rounded-tr-none' : 'bg-primary text-white rounded-tl-none shadow-md'}`}>
                            {msg.text}{msg.isPartial && <span className="inline-block w-1.5 h-3 ml-1 bg-current opacity-70 animate-pulse"/>}
                        </div>
                    </motion.div>
                ))}
            </div>
            ...
            <div className="p-5 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 shrink-0 relative">
              <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-white dark:bg-slate-800 px-3 py-1 rounded-full border border-slate-100 dark:border-slate-700 shadow-sm flex items-center gap-2">
                 {isConnected ? (
                     <><div className={`w-2 h-2 rounded-full ${isSpeaking ? 'bg-primary' : 'bg-green-500 animate-pulse'}`}></div><span className="text-[10px] font-bold uppercase text-slate-500 tracking-wide">{isSpeaking ? 'Melanie Speaking' : 'Listening...'}</span></>
                 ) : (
                  ...
                  <div className="flex items-center justify-center h-16 gap-1 mb-4 w-full">
                      {isConnected ? (
                          Array.from({ length: 12 }).map((_, i) => (
                             <div key={i} className="w-1.5 bg-gradient-to-t from-primary to-secondary rounded-full transition-all duration-75" style={{ height: `${Math.max(4, Math.min(40, volume * (Math.random() + 0.5)))}px`, opacity: volume > 2 ? 1 : 0.3 }} />
                          ))
                      ) : (
                          <div className="text-slate-400 text-xs italic">"Ask me about Local Dominance or Recruitment funnels."</div>
                      )}
                  </div>
                  <div className="w-full">
                      {!isConnected ? (
                          <Button onClick={startCall} fullWidth className="bg-primary hover:bg-blue-800 shadow-lg py-3 text-sm flex items-center justify-center gap-2"><Mic className="w-4 h-4" /> Start Strategy Session</Button>
                      ) : (
                          <div className="grid grid-cols-4 gap-2">
                              <Button variant="outline" className="col-span-1 flex items-center justify-center !px-0 border-red-200 text-red-500 hover:bg-red-50 dark:border-red-900/30" onClick={cleanup}><PhoneOff className="w-4 h-4" /></Button>
                              <div className="col-span-3"><Button fullWidth className="bg-slate-800 hover:bg-slate-900 dark:bg-slate-700 border-none shadow-md text-sm" disabled>Live Connection Active</Button></div>
                          </div>
                      )}
                  </div>
              </div>
          ...
          <motion.button initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0, opacity: 0 }} whileHover={{ scale: 1.05 }} onClick={() => setIsOpen(true)} className="fixed bottom-6 right-6 z-50 group">
             <div className="absolute -top-2 right-0 bg-secondary text-white text-[10px] font-bold px-2 py-0.5 rounded-full shadow-md animate-bounce">AI AGENT</div>
            <div className="w-16 h-16 rounded-full bg-primary text-white shadow-2xl flex items-center justify-center border-4 border-white dark:border-slate-800 group-hover:bg-blue-800 transition-colors"><Mic className="w-8 h-8" /></div>
          </motion.button>
        )}
      </AnimatePresence>
    </>
